...
function ExternalLink({ href, children, ...props }) {
  ...
  return safe ? <a href={safe} target="_blank" rel="noopener noreferrer" {...props}>{children}</a> : <span {...props}>{children}</span>;
}
...
function Header({ tab, setTab, snapshot, refreshing, onRefresh }) {
  return <>
    <header className="topbar">
      ...
      <div className="header-actions"><span className="privacy"><span className="pulse"/>{snapshot.readOnly ? 'Read-only share' : 'Local only'}</span>{snapshot.readOnly ? null : <button className="icon-button" onClick={onRefresh} disabled={refreshing} title="Refresh local project status"><Icon name="refresh"/></button>}</div>
    </header>
...
function XDigestCard({ digest, rank }) {
  ...
  return <article className="x-digest-card">
    ...
    <div className="x-digest-links">{sources.slice(0, 3).map((source, index) => <ExternalLink href={source.url} key={`${source.id}-${index}`}>Open source {index + 1}<Icon name="arrow" size={13}/></ExternalLink>)}</div>
  </article>;
}
...
function XView({ snapshot, onSync, syncing }) {
  ...
  return <main className="page"><section className="hero compact"><div><span className="eyebrow">Personal-priority intelligence</span><h1>X Posts</h1><p>Your private bookmark stream. Credentials stay server-side, and this dashboard does not send bookmark content to a hosted model.</p></div><div className="radar-controls"><Badge tone={evaluation ? 'healthy' : 'setup'}><StateDot state={evaluation ? 'healthy' : 'setup'}/>{evaluation ? `Imported synthesis · ${evaluation.model}` : 'No imported synthesis'}</Badge>{snapshot.readOnly ? null : <button className="sync-button" onClick={onSync} disabled={syncing}><Icon name="refresh" size={15}/>{syncing ? 'Syncing…' : 'Sync reports'}</button>}</div></section>
    ...
    <div className="x-grid"><section><div className="section-heading"><div><span className="eyebrow">Saved posts</span><h2>Recent bookmarks</h2></div><span>20 of {x.bookmarks?.length || 0}</span></div>{x.bookmarks?.length ? <div className="bookmark-list">{x.bookmarks.slice(0, 20).map((bookmark, index) => <ExternalLink href={bookmark.url} className="bookmark" key={`${bookmark.url}-${index}`}><Badge tone="owner">Owner saved</Badge><h3>{bookmark.text || 'X post'}</h3><span>{bookmark.author_name || (bookmark.author_username ? `@${bookmark.author_username}` : 'X')} · {formatDate(bookmark.created_at)}</span></ExternalLink>)}</div> : <div className="empty-state small"><h2>No bookmarks on this Mac yet</h2><p>Until OAuth is connected, the iPhone Share Sheet remains the free capture path.</p></div>}</section>
...
function App() {
  ...
  async function load() { const snapshotResponse = await fetch(`${BASE}data/snapshot.json?time=${Date.now()}`, { cache: 'no-store' }); if (!snapshotResponse.ok) throw new Error(`Snapshot HTTP ${snapshotResponse.status}`); setSnapshot(await snapshotResponse.json()); const fallback = { csrfToken: '', readOnly: true, aiEvaluation: { enabled: false, reason: 'API unavailable in static development mode' } }; try { const response = await fetch(`${BASE}api/session`, { cache: 'no-store' }); setSession(response.ok && response.headers.get('content-type')?.includes('application/json') ? await response.json() : fallback); } catch { setSession(fallback); } }
  useEffect(() => { load().catch((err) => setError(err.message)); }, []);
  useEffect(() => { window.location.hash = tab; }, [tab]);
  const mutationHeaders = (json = false) => ({ 'x-mc-csrf-token': session.csrfToken, ...(json ? { 'content-type': 'application/json' } : {}) });
  async function refresh() { setRefreshing(true); setError(''); try { const response = await fetch(`${BASE}api/refresh`, { method: 'POST', headers: mutationHeaders() }); if (response.ok) setSnapshot(await response.json()); else await load(); } catch { await load(); } finally { setRefreshing(false); } }
  async function syncAiradar() { setSyncing(true); setError(''); try { const response = await fetch(`${BASE}api/airadar/sync`, { method: 'POST', headers: mutationHeaders() }); if (!response.ok) throw new Error('AIRadar report sync was rejected or unavailable.'); setSnapshot(await response.json()); } catch (err) { setError(err.message); } finally { setSyncing(false); } }
  if (error) return <div className="load-screen"><h1>Observatory unavailable</h1><p>{error}</p></div>;
  if (!snapshot || !session) return <div className="load-screen"><span className="radar-orb"><i/></span><p>Powering up Observatory…</p></div>;
  async function setArchived(project, archived) { setRefreshing(true); setError(''); try { const response = await fetch(`${BASE}api/projects/${project.id}/archive`, { method: 'POST', headers: mutationHeaders(true), body: JSON.stringify({ archived }) }); if (!response.ok) throw new Error('Archive update failed'); setSnapshot(await response.json()); } catch (err) { setError(err.message); } finally { setRefreshing(false); } }
  ...
  return <div className="app"><Header tab={tab} setTab={setTab} snapshot={snapshot} refreshing={refreshing} onRefresh={refresh}/>{snapshot.sharing?.redactions?.length ? <div className="share-redaction">Shared read-only mode: project, Atlas, and AIRadar details are redacted.</div> : null}{tab === 'atlas' ? <AtlasView snapshot={snapshot}/> : tab === 'airadar' ? <AIRadarView snapshot={snapshot} onSync={syncAiradar} syncing={syncing}/> : tab === 'x' ? <XView snapshot={snapshot} onSync={syncAiradar} syncing={syncing}/> : <MissionView snapshot={snapshot} onArchive={requestArchive} onKeepArchived={(project) => setArchived(project, true)}/>}<footer><span>Observatory Mission Control · {snapshot.readOnly ? 'read-only share' : 'private on this Mac'}</span><span>Refreshed {formatDate(snapshot.generatedAt)}</span></footer>{archiveCandidate ? <ArchiveConfirmation project={archiveCandidate} onCancel={() => setArchiveCandidate(null)} onConfirm={confirmArchive}/> : null}</div>;
}
