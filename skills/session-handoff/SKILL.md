...
4. Include `## Working ownership / overlap` using `docs/concurrency-contract.md`. Record the branch and exact head, intended path scope, active refs checked, and unresolved/reconciled overlap. Run `observatory overlap` against known active refs before describing ownership as clean.
